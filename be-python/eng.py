...
app = Flask(__name__)
run_with_ngrok(app)  # Run with ngrok for testing
CORS(app)  # Allow CORS so the API can be accessed from the frontend
...
# Maximum and minimum audio file duration (seconds)
...
# Initialize the Roboflow model for plant disease detection
...
def convert_png_to_jpeg(filepath):
    ...
    try:
        ...
        logger.error(f"Failed to convert PNG image: {e}")
        return filepath
    ...
    try:
        ...
        if not weather_data:
            return jsonify({
                ...
                'description': 'Clear',
                'location': 'Jakarta',
                'advice': 'Suitable for harvesting or drying crops'
            }), 500
...
def transcribe_audio():
    ...
    try:
        ...
        # Transcribe assuming local language context
...
def analyze_image():
    ...
    try:
        ...
        logger.info(f"📊 Raw prediction result from Roboflow: {result}")

        predictions = result.get("predictions", [])
        if not predictions:
            explanation = "No disease detected in the image. Please ensure the image is clear and focused on the problematic leaf or plant area."
            return jsonify({
                "detected": None,
                "confidence": 0.0,
        ...
        # Filter only predictions that have confidence and class
        filtered_preds = [p for p in predictions if "confidence" in p and "class" in p]
        if not filtered_preds:
            explanation = "Prediction result is incomplete or invalid."
            return jsonify({
                "detected": None,
                "confidence": 0.0,
                "explanation": explanation,
        ...
        label = top_pred.get("class", "Unknown")
        confidence = round(top_pred.get("confidence", 0) * 100, 2)

        prompt = f"What is the disease '{label}' in plants and how to treat it?"
        explanation = get_deepseek_response(prompt)

        # Save to database only if session is valid
        try:
            ...
            logger.warning(f"Could not save message to session: {db_err}")
...
def get_openweather_data(lat, lon):
    ...
    try:
        url = f"https://api.openweathermap.org/data/2.5/weather?lat={lat}&lon={lon}&appid={OPENWEATHER_API_KEY}&units=metric&lang=en"
...
def get_farming_advice(weather_main):
    ...
    if any(x in weather_main for x in ['clear', 'sun']):
        ...
        return "Good for planting seedlings or pesticide spraying"
    elif any(x in weather_main for x in ['rain', 'drizzle', 'shower']):
        return "Avoid fertilization and pesticide spraying"
...
def correct_user_question(raw_question):
    try:
        correction_payload = {
            "model": "deepseek-chat",
            "messages": [
                {"role": "system", "content": "You are a smart assistant whose task is to correct user questions that have typos or are unclear. Return a clearer version of the question without additional explanation."},
...
def get_deepseek_response(prompt, session_id=None, device_id=None):
    ...
    if session_id and device_id:
        ...
        if session and session.topic:
            topic_prompt = f"The current topic of discussion is '{session.topic}'."
    ...
    try:
        ...
        # Prevent contextless "yes", "continue", etc.
        if corrected_prompt.lower().strip() in ["yes", "yeah", "continue"] and not history:
            return "There is no previous conversation context to continue. Please ask a more specific question."
        ...
        payload = {
            "model": "deepseek-chat",
            "messages": [
                {"role": "system", "content": """You are PeTaniku, an agricultural assistant expert in the fields of:
...
Help users by:
1. Providing short and clear answers to agricultural questions.
2. If the question is off-topic, politely reply:
   "Sorry, I can only help with agriculture-related topics. Is there anything I can help you with regarding plants, farm weather, or related matters?"

Response style:
- Use simple and practical language.
- Use clear formatting with separate paragraphs.
- Avoid excessive technical jargon."""}] + messages_history,
            "temperature": 0.5,
            "max_tokens": 1000
        }
...
def extract_topic_from_question(question):
    try:
        response = get_deepseek_response(
            f"What is the main topic of this question? '{question}'. Provide a short answer, maximum 3 words."
        )
